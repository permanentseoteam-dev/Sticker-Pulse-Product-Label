...
- Derive state from props, React state, and refs to your own rendered output; never read it out of the surrounding DOM, for example with `querySelectorAll` or `getBoundingClientRect` on elements the component did not render itself.
- Anything repeating — `requestAnimationFrame` loops, timers, `MutationObserver`, `ResizeObserver` — must be able to stop: give retries an end condition, skip work when nothing changed, never write into the subtree you observe, and cancel or disconnect everything in the effect cleanup.
